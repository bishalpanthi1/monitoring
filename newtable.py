...
def parse_metrics(raw_metrics):
    ...
    for line in raw_metrics.splitlines():
        ...
        if len(parts) == 2:
            ...
            # Extract the file name from the backup label (i.e., the value in backup="...")
            if 'backup=' in name_and_labels:
                # Extract value inside quotes after backup=
                start = name_and_labels.find('backup="') + len('backup="')
                end = name_and_labels.find('"', start)
                backup_file = name_and_labels[start:end]
            else:
                backup_file = name_and_labels  # If no backup, just show metric name
            ...
            parsed_data.append({
                'metric': backup_file,  # Display the backup file name
...
def index():
    ...
    return render_template_string("""
        ...
        <body>
            <h1>Prometheus Metrics Table</h1>
